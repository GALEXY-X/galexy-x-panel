...
bash <(curl -Ls https://raw.githubusercontent.com/GALEXY-X/galexy-x-panel/master/install.sh) v3.4.0
```
...
docker run -d \
  ...
  --name 3x-ui \
  ghcr.io/GALEXY-X/galexy-x-panel:latest
```
...
[صفحهٔ releases](https://github.com/GALEXY-X/galexy-x-panel/releases) دانلود کنید، آن را
...
```bash
XUI_NONINTERACTIVE=1 bash <(curl -Ls https://raw.githubusercontent.com/GALEXY-X/galexy-x-panel/master/install.sh)
```

دایرکتوری [`deploy/`](https://github.com/GALEXY-X/galexy-x-panel/tree/main/deploy) در مخزن
